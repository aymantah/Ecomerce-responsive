...
function Import() {
    ...
    const addDesign = (e) => {
        e.preventDefault();
        const uploadTask = storage.ref(`t-design-images/${designImg.name}`).put(designImg);
        uploadTask.on('state_changed', snapshot => {
            const progress = (snapshot.bytesTransferred / snapshot.totalBytes) * 100;
            console.log(progress);
        }, err => setError(err.message)
            , () => {
                storage.ref('t-design-images').child(designImg.name).getDownloadURL().then(url => {
                    db.collection('T-Design').add({
                        Name: name,
                        Email: email,
                        Adresse: adresse,
                        DesignImg: url,
